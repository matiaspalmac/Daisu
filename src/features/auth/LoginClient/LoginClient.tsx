...
export default function LoginClient() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res?.error) { toast.error("Credenciales inválidas", { duration: 5000 }) }
            else { toast.success("Inicio de sesión exitoso", { duration: 3000 }); router.push('/'); router.refresh() }
        } catch { toast.error("Ocurrió un error al iniciar sesión", { duration: 5000 }) }
        ...
        <div className="min-h-screen flex items-center justify-center p-4" style={{ background: 'var(--bg)' }}>
            <div className="w-full max-w-sm">
                <div className="text-center mb-8">
                    ...
                    <p className="text-sm" style={{ color: 'var(--text2)' }}>Conéctate con el mundo</p>
                </div>
