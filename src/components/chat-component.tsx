...
function ChatComponent() {
  ...
  if (status !== 'authenticated') {
    return <p className="text-blue-600">Please log in to use the chat.</p>;
  }

  return (
    <div className="flex flex-col h-96 bg-gradient-to-br from-blue-50 to-indigo-100">
      <div className="flex-1 overflow-y-auto p-4" id="messages">
        <ul className="space-y-2">
          {messages.map((message) => (
            <li key={message.id} className={`p-2 rounded-lg ${parseInt(message.id) % 2 === 0 ? 'bg-blue-100' : 'bg-indigo-50'}`}>
              <p className="text-gray-800 text-sm">{message.content}</p>
              <small className="text-blue-600 text-xs">{message.username}</small>
            </li>
          ))}
        </ul>
        <div ref={messagesEndRef} />
      </div>
      <form onSubmit={handleSubmit} className="border-t border-blue-200 p-2 flex bg-white" id="form">
        <Input
          type="text"
          value={inputValue}
          ...
          id="input"
        />
        <Button type="submit" className="ml-2 bg-blue-500 text-white hover:bg-blue-600">
          Enviar
        </Button>
      </form>
    </div>
